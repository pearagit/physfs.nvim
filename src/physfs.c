...
static lua_Integer lua_tointegerx(lua_State *L, int idx, int *isint) {
  ...
  if (isint)
    *isint = v != 0 || lua_type(L, idx) == LUA_TNUMBER;
  return v;
}
...
#define api(name, call)                                                        \
  do {                                                                         \
    if (!PHYSFS_##call)                                                        \
      return push_error(L, name);                                              \
  } while (0)
...
static int Lfile_eof(lua_State *L) {
  lua_pushboolean(L, PHYSFS_eof(check_file(L, 1)));
  return 1;
}

static int Lfile_flush(lua_State *L) {
  api("flush", flush(check_file(L, 1)));
  return_self(L);
}
...
static int Lfile_len(lua_State *L) {
  ...
  PHYSFS_sint64 len = PHYSFS_fileLength(file);
  if (len < 0) {
    push_error(L, "length"), lua_error(L);
  }
  lua_pushinteger(L, (lua_Integer)len);
  return 1;
}
...
static int Lfile_seek(lua_State *L) {
  api("seek", seek(check_file(L, 1), (PHYSFS_uint64)luaL_checkinteger(L, 2)));
  return_self(L);
}

static int Lfile_buffSize(lua_State *L) {
  api("bufSize",
      setBuffer(check_file(L, 1), (PHYSFS_uint64)luaL_checkinteger(L, 2)));
  return_self(L);
}
...
#define CASE(n, s, d)                                                          \
  case n:                                                                      \
    if ((r = PHYSFS_read##s(f, &u.d)))                                         \
      lua_pushinteger(L, u.d);                                                 \
    break
    CASE(0x002, UBE16, u16);
    CASE(0x004, UBE32, u32);
    CASE(0x008, UBE64, u64);
    CASE(0x012, ULE16, u16);
    CASE(0x014, ULE32, u32);
    CASE(0x018, ULE64, u64);
    CASE(0x102, SBE16, s16);
    CASE(0x104, SBE32, s32);
    CASE(0x108, SBE64, s64);
    CASE(0x112, SLE16, s16);
    CASE(0x114, SLE32, s32);
    CASE(0x118, SLE64, s64);
#undef CASE
  }
  return r;
}
...
#define CASE(n, s, t)                                                          \
  case n:                                                                      \
    return PHYSFS_write##s(f, (PHYSFS_##t)v)
...
static void open_file(lua_State *L) {
  luaL_Reg libs[] = {{"__gc", Lfile_close},
                     {"__len", Lfile_len},
...
static const char *checkfile(lua_State *L, const char *mod, PHYSFS_File **f) {
  ...
  luaL_Buffer B;
  if ((*f = PHYSFS_openRead(name)) != NULL)
    return name;
...
static int Lmkdir(lua_State *L) {
  api("mkdir", mkdir(luaL_checkstring(L, 1)));
  return_self(L);
}

static int Ldelete(lua_State *L) {
  api("delete", delete (luaL_checkstring(L, 1)));
  return_self(L);
}
...
static int Lunmount(lua_State *L) {
  api("unmount", unmount(luaL_checkstring(L, 1)));
  return 1;
}
...
#define open_funcs(name)                                                       \
  static int L##name(lua_State *L) {                                           \
    ...
    *(PHYSFS_File **)lua_newuserdata(L, sizeof(PHYSFS_File *)) = f;            \
    luaL_setmetatable(L, LFS_FILE);                                            \
    return 1;                                                                  \
  }
open_funcs(openRead) open_funcs(openWrite) open_funcs(openAppend)
#undef open_funcs
...
static int LsaneConfig(lua_State *L) {
  ...
  api("saneConfig", setSaneConfig(org, app, ext, includeCdRoms, archiveFirst));
  return_self(L);
}
...
LUALIB_API int luaopen_physfs(lua_State *L) {
  luaL_Reg libs[] = {{"close", Lfile_close},
